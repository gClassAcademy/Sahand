...
public class gVisitor extends AbstractVoidVisitorAdapter<Classs> {
    ...
    public void visit(MethodDeclaration declaration, Classs classs) 
    {
        ...
        if (current_Method != null)
        {
	    	...
	    	//finding all FieldAccessExpr (internal,...)
	        List<FieldAccessExpr> FieldAccessExprs = declaration.findAll(FieldAccessExpr.class);
	        for (FieldAccessExpr field : FieldAccessExprs)
	        {    
	        	AttributeAccess attributeaccess = new AttributeAccess();
	        	attributeaccess.setMethod(current_Method);
	        	attributeaccess.setCdate(Instant.now().getNano());
	        	attributeaccess.setFieldname(field.getNameAsString());  
	        	attributeaccess.setBody(field.getParentNode().get().toString());        	
	        	
	        	//finding scope of Field
	            try {
	                String x = field.getScope().calculateResolvedType().describe();
	                attributeaccess.setFullpath(x + "." + field.getNameAsString());         
	                
	                // internal class Field Access
	                if (x.equals(current_class.getFullpath()))
	                {
	            		//finding attribute Object
	                	Attribute current_Attribute=null;
	                    for (Attribute attribute : allAttributes)
	                    	if (attribute.getFullpath().equals(current_class.getFullpath() + "." + declaration.getSignature())) {
	                    		current_Attribute=attribute;
	                    		break;
	                    	}
	                    attributeaccess.setAttribute(current_Attribute);
	                    attributeaccess.setAccessscope(AccessScope.ClassInternal);
	                }
	                
	                //Internal Project Field Access
	                else
	                {
	                	attributeaccess.setAttribute(null);
	                    attributeaccess.setAccessscope(AccessScope.ClassExternal);
	                }
	            }
	            catch (Exception e) {
	            	
	            	// External Project Field Acess
	            	attributeaccess.setAccessscope(AccessScope.projectExternal);
	            	attributeaccess.setAttribute(null);
	                attributeaccess.setFullpath(field.getScope().toString() + "." + field.getNameAsString());         
				}
	            
	            
		    	//SAVE AttributeAccess into DBMS
	            attributeaccess_Repository.save(attributeaccess);
            
	        } // end of finding all FieldAccessExpr
